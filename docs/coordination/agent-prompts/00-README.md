...
git worktree add -b agent/04-ledger-core-sprint-001 \
  ../naqdengi--04-ledger-core origin/develop

# 2. Enter it
cd ../naqdengi--04-ledger-core

# 3. Identify the agent for the ownership hook
export NAQDENGI_AGENT=ledger-core
...
# 1. Create the worktree on a fresh branch from origin/develop
git worktree add -b agent/04-ledger-core-sprint-001 `
  ..\naqdengi--04-ledger-core origin/develop

# 2. Enter it
cd ..\naqdengi--04-ledger-core

# 3. Identify the agent for the ownership hook
$env:NAQDENGI_AGENT = "ledger-core"
...
| # | Agent | Slug (NAQDENGI_AGENT) | Worktree dir (sibling of repo) | Branch (sprint 001 example) |
|---|-------|--------------------|---------------------------------|------------------------------|
| 1 | ARCHITECT | `architect` | `naqdengi--01-architect` | `agent/01-architect-sprint-001` |
| 2 | PLATFORM-GUARDIAN | `platform-guardian` | `naqdengi--02-platform-guardian` | `agent/02-platform-guardian-sprint-001` |
| 3 | AUTH-TENANCY | `auth-tenancy` | `naqdengi--03-auth-tenancy` | `agent/03-auth-tenancy-sprint-001` |
| 4 | LEDGER-CORE | `ledger-core` | `naqdengi--04-ledger-core` | `agent/04-ledger-core-sprint-001` |
| 5 | SALES-AR | `sales-ar` | `naqdengi--05-sales-ar` | `agent/05-sales-ar-sprint-001` |
| 6 | PROCUREMENT-AP | `procurement-ap` | `naqdengi--06-procurement-ap` | `agent/06-procurement-ap-sprint-001` |
| 7 | BANKING-RECON | `banking-recon` | `naqdengi--07-banking-recon` | `agent/07-banking-recon-sprint-001` |
| 8 | DOCUMENTS-AI-SAFETY | `documents-ai-safety` | `naqdengi--08-documents-ai-safety` | `agent/08-documents-ai-safety-sprint-001` |
| 9 | REPORTING-ANALYTICS | `reporting-analytics` | `naqdengi--09-reporting-analytics` | `agent/09-reporting-analytics-sprint-001` |
| 10 | FRONTEND-UX | `frontend-ux` | `naqdengi--10-frontend-ux` | `agent/10-frontend-ux-sprint-001` |
| 11 | QA-AUDITOR | `qa-auditor` | `naqdengi--11-qa-auditor` | `agent/11-qa-auditor-sprint-001` |
| 12 | GITKEEPER-INTEGRATOR | `gitkeeper-integrator` | `naqdengi--12-gitkeeper-integrator` | `agent/12-gitkeeper-integrator-sprint-001` |
...
```bash
cd /path/to/naqdengi
git worktree remove ../naqdengi--04-ledger-core
