...
| Real PostgreSQL is available for invariant tests | Sophia pod cluster `14/main` online, `PostgreSQL 14.24`, databases `afaq_dev` + `afaq_test` reachable over TCP as role `afaq` |
...
A **working double-entry ledger** for Afaq Books SaaS: chart of
...
- **C7.1** — The pod is a **build and test environment**, not a source
  of truth. `/workspace/repos/afaq-books-saas` is a clone; the
